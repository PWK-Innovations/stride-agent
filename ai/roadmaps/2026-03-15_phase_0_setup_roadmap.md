...
- [ ] Verify README.md was not overwritten (restore from git if needed)
- [ ] Merge Next.js entries (`.next/`, `out/`) into existing .gitignore
- [ ] Install LangChain deps: `npm install langchain @langchain/openai @langchain/langgraph @langchain/core @langchain/tavily @langchain/classic zod mathjs pino`
- [ ] Install dev deps: `npm install -D pino-pretty`
- [ ] Create directories: `src/lib/tools/`, `src/lib/rag/`, `src/lib/utils/`, `src/app/api/chat/`, `src/app/components/`, `docs/`, `scripts/`
- [ ] Create `.env.local` with `OPENAI_API_KEY` and `TAVILY_API_KEY`
- [ ] Create `.env.example` with placeholder values (committed to git)
- [ ] Create `scripts/dev.sh` — runs `npm run dev`
- [ ] Create `scripts/build.sh` — runs `npm run build` with exit codes + JSON output
- [ ] Make scripts executable: `chmod +x scripts/dev.sh scripts/build.sh`
